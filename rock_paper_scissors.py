...
# todo eaasier to map from int: string
# CHOICES = {'rock': 1, 'paper': 2, 'scissors': 3, 'spock': 4, 'lizard': 5}
CHOICES = {1: 'rock', 2: 'paper', 3: 'scissors', 4: 'spock', 5: 'lizard'}
RULES = {
        # todo if immutable better use tuples
        # maybe bette to map int: Tuple[int]
...
class Player:
    ...
    def make_a_move(self, choice):
        # todo when CHOICES would map from int: string this would be trivial
        #move = {key for key, value in CHOICES.items() if value == choice}
        if not choice in CHOICES:
            raise ValueError("Invlid choice")
    ...
    def make_a_choice(self):
        while True:
            ...
            try:
                ...
                if 1 <= choice <= 5: # todo 1 <= choice <= 5 or len(CHOICES.keys())
                    break
                else:
...
# player2 = ComputerPlayer()# PLAY AREA -> FUNCTIONS CALL
